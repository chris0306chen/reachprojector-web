...
import { getRFQCopy } from '@/lib/rfq-copy'
...
const COUNTRIES = [
  ...
  ['Italy', 'IT'], ['Netherlands', 'NL'], ['Poland', 'PL'], ['Brazil', 'BR'], ['Mexico', 'MX'], ['Argentina', 'AR'],
  ['Saudi Arabia', 'SA'], ['UAE', 'AE'], ['Egypt', 'EG'], ['South Africa', 'ZA'], ['Nigeria', 'NG'],
  ['Turkey', 'TR'], ['Russia', 'RU'], ['India', 'IN'], ['Indonesia', 'ID'], ['Thailand', 'TH'], ['Vietnam', 'VN'],
  ['Philippines', 'PH'], ['Malaysia', 'MY'], ['Singapore', 'SG'], ['South Korea', 'KR'], ['Japan', 'JP'],
  ['Australia', 'AU'], ['Other', ''],
] as const
...
export default function RFQForm({ product, onClose, variant = 'modal' }: RFQFormProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.companyName.trim()) newErrors.companyName = copy.errors.company
    if (!formData.contactName.trim()) newErrors.contactName = copy.errors.contact
    if (!formData.country) newErrors.country = copy.errors.country
    ...
    if (!formData.phone.trim() && !formData.whatsapp?.trim()) {
      newErrors.phone = copy.errors.phone
    }
    if (formData.quantity < 1) newErrors.quantity = copy.errors.quantity
    if (!formData.intendedUse) newErrors.intendedUse = copy.errors.intendedUse
    setErrors(newErrors)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await res.json() as { data?: { rfq_number?: string } }
      setReferenceNumber(result.data?.rfq_number || null)
      setStatus('success')
      // Open WhatsApp as backup channel
      const waMsg = `${copy.title}\n${copy.success.product}: ${formData.productName || '—'}\n${copy.quantity}: ${formData.quantity}\n${copy.companyName}: ${formData.companyName}\n${copy.contactName}: ${formData.contactName}\n${copy.country}: ${formData.country}\n${copy.email}: ${formData.email}`
  ...
  if (status === 'success') {
    return <RFQSuccess formData={formData} onClose={onClose} copy={copy.success} referenceNumber={referenceNumber} />
  }
    ...
    <div className={variant === 'modal' ? 'fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4' : ''}>
      <div className={`bg-white rounded-2xl shadow-2xl ${variant === 'modal' ? 'max-w-2xl w-full max-h-[90vh] overflow-y-auto' : 'w-full'}`}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          ...
          {onClose && (
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors" aria-label={copy.close}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {copy.intendedUse} <span className="text-red-500">*</span>
            </label>
            <select
            ...
            >
              <option value="">{copy.selectIntendedUse}</option>
              {INTENDED_USES.map((use, index) => (
                <option key={use} value={use}>{copy.uses[index]}</option>
              ))}
            </select>
          ...
          <div className="border-t border-gray-100 pt-5">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">{copy.companyInfo}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {copy.companyName} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                >
                  ...
                  {COUNTRIES.map(([value, code]) => (
                    <option key={value} value={value}>{code ? regionNames.of(code) : copy.uses[7]}</option>
                  ))}
                </select>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{copy.whatsapp}</label>
                <input
                  type="tel"
          ...
          <div className="text-center">
            <span className="text-sm text-gray-400">{copy.or}</span>
          </div>
          <a
            href={`https://wa.me/8615860330104?text=${encodeURIComponent(`${copy.title}\n${copy.success.product}: ${formData.productName || '—'}\n${copy.quantity}: ${formData.quantity}`)}`}
            target="_blank"
